...
| `gatherings/new.vue` | `/gatherings/new` | `gatherings/{pushId}` (create; `?id=` edits in place) then `userGatherings/*` index sync, `users/{uid}` (own prefill incl. address + `savedLocations` read/write), `users/{friendId}/collection` (each friend's, so the Games select can also offer their games — mutual-friends rule), `profiles/{uid}/name` (friend/guest names) |
| `index.vue`          | `/`               | none                                                                                                                                                              |
...
- `helpers/gatherings.ts` — `splitGatherings` (upcoming hosting/invited + a `past` section; a gathering turns past once `datetime` + the 3-hour play window elapses, see `isPast`), state/response color+icon maps, `formatDatetime`, `buildGameItems` (merges the host's own collection with friends' collections into the gathering form's Games select — friend-only games get the friend's name as the list item's `subtitle`; games the host also owns have no subtitle)
...
- `CookieConsent.vue` / `InstallBanner.vue` — fixed dismissible bottom banners rendered from `layouts/default.vue`; see Privacy/cookie consent and PWA & push notifications below for their respective composables
...
- `useServiceWorker.ts` — registers `public/firebase-messaging-sw.js` (module-scoped, shared registration); see PWA & push notifications below
- `useInstallPrompt.ts` — captures `beforeinstallprompt` (Android/Chrome only), exposes `canInstall`/`installed`/`promptInstall()`
- `useNotifications.ts` — notification permission + FCM token lifecycle: `permission`, `enabled` (permission granted **and** a token is on file — these can diverge, see the file), `enable()`, `disable()`
...
// users/{uid}/fcmTokens/{token}: number — owner-only; keyed by the FCM
// registration token itself (re-registering the same device overwrites its
// own entry rather than accumulating duplicates), value is the epoch-ms
// write time. Written by useNotifications.ts when the user enables push
// notifications on /profile; read by the Cloud Functions push helper
// (sendPushToUser in functions/src/index.ts) and pruned there when FCM
// reports a token as no-longer-registered. See PWA & push notifications below.
...
// emailInviteIndex/{sha256(email.toLowerCase())}/{gatheringId}: true — Cloud-Functions-only,
// no client .read/.write rule at all (see Firebase Security Rules below). Lets a guest whose
// verified sign-in email matches an emailInvites entry discover that gathering even without
// following the exact Accept/Decline link — see listMyEmailInvites. Written by
// onGatheringEmailInvite when a host adds an email invite, deleted by acceptEmailInvite when
// consumed and by onGatheringEmailInviteRemoved when the emailInvites entry is removed any
// other way (host un-invites, gathering deleted, host account deleted)

// emailInviteRateLimit/{hostUid}: { windowStart: number, count: number } — Cloud-Functions-only,
// no client .read/.write rule at all (same reasoning as emailInviteIndex — a client that could
// read or reset its own counter could defeat it). Throttles how many email invites one account
// can send per hour (see nextEmailInviteRateLimitState, EMAIL_INVITE_RATE_LIMIT_* in
// functions); checked and incremented atomically inside onGatheringEmailInvite before it sends
// anything or writes to emailInviteIndex
```

## Firebase Security Rules

`database.rules.json` covers `profiles/`, `users/`, `friendRequests/`, `blocked/`, `gatherings/`, `userGatherings/`, `emailInviteIndex/`, and `emailInviteRateLimit/` (deployed automatically by `cd.yml` on push to `main`; manual deploy: `firebase deploy --only database`). All nodes reject unknown keys via `"$other": { ".validate": false }` and bound types/lengths with field-level `.validate` rules.
...
- `users/{uid}` — owner-only read **and** write for phone, address, maxPeople, `savedLocations` (past gathering addresses), `gameOpinions` (ratings + `privateNote`), `fcmTokens` (push registration tokens, see PWA & push notifications below), and the friends list — with one carve-out: `users/{uid}/collection` is additionally readable by *mutual* friends (both sides present in each other's friends lists), which backs the friend-collection view.
...
- `emailInviteIndex/` — declared with `".read": false, ".write": false` and nothing else, so it's sealed to every client at every depth (this is on top of, not instead of, the root's own default deny — belt and suspenders, and self-documenting that the omission is deliberate rather than forgotten). Reads and writes happen exclusively through the Admin SDK inside `onGatheringEmailInvite`, `onGatheringEmailInviteRemoved`, `acceptEmailInvite`, and `listMyEmailInvites`. **This must never gain a `.read` or `.write` rule** — the index exists specifically so no client can enumerate "who's invited where" by email; see the security-rules test asserting this in `test/rules/database.rules.spec.ts`.
- `emailInviteRateLimit/` — same sealing as `emailInviteIndex`, same reason it must stay that way: a client able to read or write its own counter could inflate or reset it, defeating the throttle. Touched only by `tryConsumeEmailInviteQuota` inside `onGatheringEmailInvite`.
...
- **`emailInviteIndex` widens delivery, not data**: a host entering any address already causes an invite email to be sent to it; the index additionally surfaces that same gathering (via `listMyEmailInvites`) in-app for whichever account's *verified* email matches, without that person clicking anything. No new information is exposed beyond what the email itself already contains — it's a second delivery path for an invite that was always going to reach that address, not a new way to see someone else's data.
- **`emailInviteIndex` isn't reference-counted**: it's keyed `(emailHash, gatheringId)`, a single boolean, even though `gatherings/{id}/emailInvites` could in principle hold more than one entry for the same address (e.g. two browser tabs concurrently editing and saving the same gathering — the client-side add/remove diffing in `gatherings/new.vue` already no-ops a same-session remove-then-re-add, so this needs genuine concurrent saves). Removing any one of the duplicate `emailInvites` entries deletes the shared index entry outright, which can make a still-valid invite briefly undiscoverable via `listMyEmailInvites` — though the original deep-link email for the surviving entry still works, since `acceptEmailInvite` searches `emailInvites` directly rather than going through the index. Narrow enough (and low-stakes enough — discovery, not data loss) not to warrant turning the index value into a ref-counted map.
...
- **Email invite for non-users**: the gathering form has an "Email invites" section where the host can enter any email address (friend not required), capped at `MAX_EMAIL_INVITES` (20, matching `EMAIL_INVITE_RATE_LIMIT_MAX` below) per gathering so one big invite round can't by itself exceed the account-wide quota and get some invites silently dropped. Each address is stored in `gatherings/{id}/emailInvites/{pushId}`. `onGatheringEmailInvite` fires — checked against the sending host's quota first (see below), then writes a hashed lookup entry to `emailInviteIndex` (see below), and sends an invite email with RSVP deep-links. When the recipient signs in and follows the link, `calendar.vue` detects they aren't yet a guest and calls the `acceptEmailInvite` Cloud Function, which verifies their email against `emailInvites`, writes `guests/{uid}` + `userGatherings/{uid}` as an atomic admin update (bypassing the mutual-friendship rule), and removes the consumed email invite entry (and its `emailInviteIndex` entry). The existing RSVP watcher then finishes applying the response.
- **Email invite rate limiting**: unlike inviting an existing friend, email invites need no prior relationship — the one path in the app that emails (and now surfaces a gathering to) someone who never agreed to anything, so it's the one path worth throttling. Enforced in `onGatheringEmailInvite` via `tryConsumeEmailInviteQuota`, since RTDB rules have no server-time primitive and can't express this declaratively. Over quota, the function silently returns (same as the existing invalid-email-format case) rather than rejecting the `emailInvites` write, so a host is never left thinking a save failed. The window/threshold values, the fixed-window-with-lazy-reset semantics, and the accepted retry/permanent-failure tradeoffs are documented where they live — `functions/src/emailInviteRateLimit.ts` and the comments on `tryConsumeEmailInviteQuota` in `functions/src/index.ts` — rather than re-derived here.
- **Finding an email invite without the exact link**: someone who signs up separately, loses the invite email, or follows the plain "View on your calendar" link instead of Accept/Decline would otherwise never see it — RTDB can't query "every gathering that invited my email" across the tree. `emailInviteIndex/{sha256(email)}/{gatheringId}: true` solves this: `onGatheringEmailInvite` writes an entry alongside sending the email, and `calendar.vue` calls the `listMyEmailInvites` Cloud Function once per load (after any deep-link RSVP has been applied, so an invite just accepted that way doesn't also show up here) to render a "Pending invites" section above Hosting/Invited. Accept/Decline there call the same `acceptEmailInvite` function — this only adds a way to *find* an invite, not a new way to accept one. The index carries no database rule of its own (Cloud-Functions-only; see Firebase Security Rules above) — `listMyEmailInvites` hashes the caller's own verified token email server-side and can only ever read that one bucket, caps the fan-out at `MAX_EMAIL_INVITES_PER_LOOKUP` (25 — nothing rule-enforces how many gatherings can index one address, so this bounds the read cost of a single call), treats each entry independently (one bad lookup can't blank out the others), and skips — and self-heals — any entry for a gathering where the caller is already a real guest through some other path, since surfacing it would let its Accept/Decline silently overwrite an existing response.

## PWA & push notifications

The app is installable on Android/Chrome and can send web push notifications via Firebase Cloud Messaging (FCM). Both features share one service worker.

- **`public/firebase-messaging-sw.js`** — registered at the origin root (scope covers the whole app) by `useServiceWorker.ts`, called unconditionally on boot from `plugins/04-service-worker.client.ts` (not gated on cookie consent — a controlling service worker is one of Chrome's install-prompt criteria, so this needs to run before the visitor opts into anything). It does double duty: `self.addEventListener('install'/'activate', …)` plus a controlling SW is what makes the app installable, and `firebase.messaging().onBackgroundMessage(...)` handles push while the app isn't focused, showing a native notification whose click deep-links back into the app (`notificationclick` reads the `link` stashed on the notification's `data`). `public/` files aren't Vite-processed, so unlike the rest of the client, Firebase config here can't be build-templated — non-secret fields (`projectId`, `databaseURL`, `messagingSenderId`, …) are hardcoded in the file, and `apiKey`/`appId` travel as query params on the registration URL (`useServiceWorker.ts` builds it from `firebase.config.ts`), which is why they only ever appear once a client actually calls `register()`.
- **Install prompt (Android/Chrome only)**: `useInstallPrompt.ts` captures the `beforeinstallprompt` event (module-scoped state, listener attached at boot by `plugins/05-install-prompt.client.ts` since Chrome fires it once per qualifying page load rather than on demand — waiting until the user visits `/profile` would miss it). `pages/profile.vue` shows an "Install app" button in its "App" section when `canInstall` is true; there's no iOS equivalent (`beforeinstallprompt` is Chromium-only), so the button simply never appears there. `components/InstallBanner.vue` additionally surfaces this proactively as a dismissible bottom banner (mirroring `CookieConsent.vue`'s style, rendered from `layouts/default.vue`) so users don't have to already know to look in Profile — "Not now" persists via `bgc-install-banner-dismissed` in localStorage (`bannerDismissed`/`dismissBanner` in the composable) and doesn't affect the profile page's own button, which stays available regardless. The banner defers to the cookie-consent banner (`cookieConsentAnswered` from `useCookieConsent`) so the two fixed bottom banners never stack on a first-time visit; it appears the moment the visitor answers cookies, if `beforeinstallprompt` already fired. It's also gated to viewports under `960px` (a `matchMedia` check, same split the game-item list uses for its own mobile/desktop switch) — desktop Chrome/Edge already show a native install icon in the address bar, so the banner would just be redundant nagging there; the Profile button still works on desktop regardless.
- **Push notifications**: `useNotifications.ts` handles the client side — `enable()` requests `Notification` permission, gets an FCM token (`getToken`, needing the `FCM_VAPID_KEY` public runtime config — set from a Firebase web push certificate; see `.env.example` and `cd.yml`), and writes it to `users/{uid}/fcmTokens/{token}` (rule: owner read/write, value must be a positive number — the write timestamp). `disable()` deletes that token; browser permission itself can't be revoked programmatically, so `enabled` is `permission === 'granted' && tokenSaved` (a separately-tracked ref), not permission alone — otherwise "Turn off" would have no visible effect. Foreground messages (app focused) don't go through the service worker's `onBackgroundMessage`; `onMessage` in this composable shows a native `Notification` directly instead, to avoid plumbing a global toast bus through every page for what's otherwise just `<Snackbar>`'s job.
- **Sending pushes**: `functions/src/index.ts`'s `sendPushToUser(uid, { title, body, link })` reads `users/{uid}/fcmTokens`, fans a notification out via `getMessaging().sendEachForMulticast`, and deletes any token FCM reports as no-longer-registered. It's called from the same four triggers that send email (`onFriendRequest`, `onGatheringInvite`, `onGuestResponse`, `onGatheringStateChange`) but is a best-effort companion, not part of the email retry logic above — every call site wraps it in `.catch(console.error)` so a push failure never throws and never affects the email `retry: true` semantics. `onGatheringEmailInvite` does not send a push (the recipient may not have an account, hence no `fcmTokens` to read).
- **Icons**: `public/icons/icon-{192,512}.png` and `icon-maskable-512.png`, rendered from the same meeple mark as `favicon.svg`, referenced from `public/manifest.webmanifest`. Regenerate by re-rasterizing the SVG (e.g. a headless-Chromium screenshot at the target size) if the mark ever changes — there's no build step that does this automatically.
...
`gamecollection.vue` is the reference pattern: text filter + sort `v-select` toolbar, `v-chip-group multiple` for genre facets (`color="info"`; use the group — not hand-rolled `aria-pressed` — for correct keyboard nav), "Showing N of M" count, then a **virtualized list** (`v-virtual-scroll`, no `height`/`max-height` — never a plain `v-list`). Leaving height unset is deliberate: `v-virtual-scroll` falls back to virtualizing against the nearest scrolling ancestor, which is the document itself here, so the page has exactly one scrollbar instead of a nested one (a fixed `max-height` created a second, inner scrollbar — a past bug). Genres beyond `GENRE_CHIP_LIMIT` (12) collapse behind a "+N more" toggle. Do not add per-row entry animations on virtualized rows — they replay on every scroll recycle. Filter/sort lives in `helpers/collection.ts`; returns an **ordered array**, not a keyed object.

Each row (`.game-item`, shared by the collection list and the "Also rated" list) stacks name/chips → rating → meta text → actions vertically on mobile, but at `md` and up (`min-width: 960px`) becomes a single flex row — name/chips (`.game-item-info`, `flex: 1 1 260px`) grow, rating/meta stay fixed-width, actions (`.game-item-actions`) get `margin-left: auto` to pin to the row's end, and the note textarea (`.game-item-note`) forces a new line with `flex: 1 1 100%` when expanded. This is the reference pattern for any other dense list that looks stacked-and-empty on wide viewports — target `.v-list-item__content` (which wraps a `v-list-item`'s whole default slot) with `display: flex; flex-wrap: wrap` behind the same media query.
