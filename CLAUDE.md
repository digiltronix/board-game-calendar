...
- `helpers/gatherings.ts` — `splitGatherings` (upcoming hosting/invited + a `past` section; a gathering turns past once `datetime` + the 3-hour play window elapses, see `isPast`), state/response color+icon maps, `formatDatetime`, `buildGameItems` (merges the host's own collection with friends' collections into the gathering form's Games select — friend-only games are suffixed `(Friend Name)`; games the host also owns are listed unsuffixed)
