...
// can send per rolling hour (see tryConsumeEmailInviteQuota, EMAIL_INVITE_RATE_LIMIT_* in
...
- **Email invite for non-users**: the gathering form has an "Email invites" section where the host can enter any email address (friend not required). Each address is stored in `gatherings/{id}/emailInvites/{pushId}`. `onGatheringEmailInvite` fires — checked against the sending host's rolling-hour quota first (see below), then writes a hashed lookup entry to `emailInviteIndex` (see below), and sends an invite email with RSVP deep-links. When the recipient signs in and follows the link, `calendar.vue` detects they aren't yet a guest and calls the `acceptEmailInvite` Cloud Function, which verifies their email against `emailInvites`, writes `guests/{uid}` + `userGatherings/{uid}` as an atomic admin update (bypassing the mutual-friendship rule), and removes the consumed email invite entry (and its `emailInviteIndex` entry). The existing RSVP watcher then finishes applying the response.
- **Email invite rate limiting**: unlike inviting an existing friend, email invites need no prior relationship — the one path in the app that emails (and now surfaces a gathering to) someone who never agreed to anything, so it's the one path worth throttling. `tryConsumeEmailInviteQuota` (in `functions/src/index.ts`) atomically checks and increments `emailInviteRateLimit/{hostUid}` inside `onGatheringEmailInvite`, before it does anything else — `EMAIL_INVITE_RATE_LIMIT_MAX` (20) per `EMAIL_INVITE_RATE_LIMIT_WINDOW_MS` (1 hour), generous enough that a real host sending real invites for a real gathering never notices. Over quota: the function silently returns (same as the existing invalid-email-format case) — the host's `emailInvites` write itself still succeeds, it just doesn't get acted on, so a host isn't left thinking a save failed. RTDB rules have no server-time primitive, so a rolling window can't be expressed declaratively; this has to be enforced in the function. Known tradeoff: because `onGatheringEmailInvite` sets `retry: true`, a *retried* delivery after a transient Resend failure re-consumes quota for what's logically the same invite — accepted rather than adding a per-invite idempotency marker, since transient failures are uncommon and the limit has headroom to absorb a few.
